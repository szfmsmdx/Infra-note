...
## Event
是一个简单的二值信号量，初始状态为”unset/False“，允许一个或多个进程等待某个事件发生，另一个进程在条件满足的时候 set 该 event，从而唤醒等待者

常用方法

| 方法                                | 说明                                                              |
| --------------------------------- | --------------------------------------------------------------- |
| `event = multiprocessing.Event()` | 创建一个新的 Event 对象，初始状态为 **False（未设置）**                            |
| `event.set()`                     | 将事件状态设为 **True（已设置）**，并**唤醒所有等待该事件的进程**                         |
| `event.clear()`                   | 将事件状态重置为 **False（未设置）**                                         |
| `event.is_set()`                  | 返回当前事件状态（`True` 或 `False`）                                      |
| `event.wait(timeout=None)`        | **阻塞当前进程**，直到事件被 `set()`；若指定 `timeout`（秒），则最多等待那么久，超时返回 `False` |

我们可以利用 event 设计一个进程顺序执行的操作过程：
```python
import multiprocessing as mp

def worker(rank, next_event, current_event):
    # 如果不是 0 号进程，先等待前一个进程的信号
    if current_event is not None:
        current_event.wait()  # 等待“轮到我”的信号
    
    print(f"Process {rank}")
    
    # 通知下一个进程可以执行（如果不是最后一个）
    if next_event is not None:
        next_event.set()

if __name__ == "__main__":
    num_processes = 5
    # 创建 4 个 Event：用于 0→1, 1→2, 2→3, 3→4 的通知
    events = [mp.Event() for _ in range(num_processes - 1)]
    
    processes = []
    for rank in range(num_processes):
        # 当前进程需要等待的 event（rank=0 时为 None）
        current_event = events[rank - 1] if rank > 0 else None
        # 当前进程完成后要触发的 event（rank=4 时为 None）
        next_event = events[rank] if rank < num_processes - 1 else None
        
        p = mp.Process(target=worker, args=(rank, next_event, current_event))
        processes.append(p)
        p.start()
